...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    val lm = if (left.isEmpty) elem else left.mostRetweeted
    val rm = if (right.isEmpty) elem else right.mostRetweeted
    val greater = if (rm.retweets > elem.retweets) rm else elem
    if (lm.retweets > greater.retweets) lm else greater
  }
